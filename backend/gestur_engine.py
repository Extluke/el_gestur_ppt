...
class GestureEngine:
    ...
    def process_frame(self, frame, roi=None):
        ...
        if roi:
            ...
            process_area, area_w, area_h = frame[y1:y2, x1:x2], x2 - x1, y2 - y1
        else: process_area, area_w, area_h = frame, w, h

        results = self.hands.process(cv2.cvtColor(process_area, cv2.COLOR_BGR2RGB))

        if results.multi_hand_landmarks:
            for hand_landmarks, hand_info in zip(results.multi_hand_landmarks, results.multi_handedness):
                hand_label = "Left" if hand_info.classification[0].label == "Right" else "Right"
                if roi:
                    for lm in hand_landmarks.landmark: cv2.circle(frame, (int(lm.x * area_w) + x1, int(lm.y * area_h) + y1), 4, (0, 255, 0), -1)
                else: mp_draw.draw_landmarks(frame, hand_landmarks, mp_hands.HAND_CONNECTIONS, mp_styles.get_default_hand_landmarks_style(), mp_styles.get_default_hand_connections_style())
